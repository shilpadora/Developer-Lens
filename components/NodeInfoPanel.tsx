  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-300 overflow-hidden bg-slate-900/5">
      <div className="p-8 border-b border-slate-800 bg-slate-900/50 flex flex-col gap-6">
        {!node ? (
          <div className="text-center py-20 opacity-30 w-full flex flex-col items-center select-none">
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase mt-2">Select a node in the map to activate</p>
          </div>
        ) : (
          <div className="flex flex-col gap-4 w-full">
            <div className="flex items-center justify-between">
               <div className="flex items-center gap-3 overflow-hidden">
                 <div className={`flex-shrink-0 w-3.5 h-3.5 rounded-full ${node.complexity === 'high' ? 'bg-red-500' : node.complexity === 'medium' ? 'bg-orange-500' : 'bg-emerald-500'} shadow-[0_0_15px_rgba(16,185,129,0.3)]`}></div>
                 <h2 className="text-2xl font-black uppercase tracking-tighter truncate text-white">{node.name}</h2>
               </div>
            </div>
            <div className="flex items-center gap-3">
              <p className="text-[10px] font-mono text-slate-400 truncate bg-slate-950/80 p-3 rounded-xl border border-slate-800/50 flex-1">{node.path}</p>
              <a href={githubUrl} target="_blank" rel="noopener noreferrer" className="p-3 bg-slate-800 hover:bg-emerald-500 text-slate-950 rounded-xl border border-slate-700 transition-all flex items-center gap-2 group" title="Open in GitHub">
      ...
      {node && (
        <>
          <div className="flex border-b border-slate-800 bg-slate-950/40 sticky top-0 z-10">
            ...
            <TabBtn active={activeTab === 'contributors'} onClick={() => setActiveTab('contributors')} icon={<Users size={16}/>} label="History" />
            <TabBtn active={activeTab === 'audit'} onClick={() => setActiveTab('audit')} icon={<Zap size={16}/>} label="Intelligence" />
            <TabBtn active={activeTab === 'chat'} onClick={() => setActiveTab('chat')} icon={<MessageSquare size={16}/>} label="Consult" />
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            {activeTab === 'node' && (
              <div className="space-y-10 animate-in slide-in-from-bottom-5 duration-300">
                 ...
                 <InfoSec label="Complexity Rating" value={node.complexity.toUpperCase()} icon={<Activity size={16}/>} />
                 {node.size && <InfoSec label="Disk Size" value={`${(node.size/1024).toFixed(2)} KB`} icon={<Code size={16}/>} />}
              </div>
            )}
